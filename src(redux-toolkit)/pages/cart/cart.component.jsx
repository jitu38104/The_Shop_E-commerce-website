...
import { deleteCartItem, addCartItem, removeCartItem } from "../../store/cart/cart.reducer";
...
const Cart = () => {
    ...
    const removeCartItemHandle = (item) => {
        if(item?.qty > 1) dispatch(removeCartItem(item));
    }

    const addCartItemHandle = (item) => dispatch(addCartItem(item));

    const deleteCartItemHandle = (id) => dispatch(deleteCartItem(id));
